...
class SiteOrigin_Widget_Button_Widget extends SiteOrigin_Widget {
	...
	add_filter( 'siteorigin_widgets_template_file_sow-button', array($this,'mytheme_button_template_file'),10, 3 );	
	}
				...
				'fields' => array(
					...
					'theme' => array(
						...
						'options' => array(
							...
							'mystyle' => __( 'MyStyle', 'so-widgets-bundle' ),
						),
					),
	...
	function mytheme_button_template_file( $filename, $instance, $widget ) {
		...
		if ( ! empty($instance['design']['theme']) && $instance['design']['theme'] == 'mystyle' ) {
			// This option works for plugins.
			$filename = plugin_dir_path( __FILE__ ) . 'tpl/mystyle.php';
	
			// And this one for themes.
			// $filename = get_stylesheet_directory() . '/tpl/mystyle.php'; 
		}
		return $filename;
	}
